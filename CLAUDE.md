...
| 사용자가 혼자 고치는 본문 텍스트 | 4개 HTML 안의 `<!-- ✏️ 수정구역: 이름 -->` ~ `<!-- ✏️ 수정구역 끝 -->` 주석 쌍 (2026-07-16 도입 — 본편 3파일 각 34개, 세 파일에서 이름·구조 위치 동일 + index.html 6개). 전체 목록은 루트 `나중에 나 혼자 볼 설명서.md` 4번 |
...
14. **셀프 업데이트 시스템** (2026-07-16): 사용자가 구독 없이 혼자 사이트를 고치도록 만든 세 부품. ① 4개 HTML의 `✏️ 수정구역` 주석 쌍 — 본편 각 34개(세 언어 파일에서 같은 이름이 같은 구조 위치) + index.html 6개. ② 원클릭 배포 `업데이트.bat`(ASCII 전용 런처) + `update_site.ps1`(UTF-8 BOM, 한국어 안내; `git add -A`로 pending 전부를 커밋·push한다 — 세션을 clean으로 끝내야 하는 또 하나의 이유). ③ 사용자용 `나중에 나 혼자 볼 설명서.md`. 유지 규칙: 수정구역 주석을 지우거나 이름을 바꾸지 마라 — 설명서의 목록·Ctrl+F 검색어와 짝이다. 콘텐츠 블록을 추가/삭제하면 세 파일에 같은 이름의 수정구역을 함께 넣고 설명서 4번 목록도 갱신한다. `업데이트.bat`에는 비ASCII 문자 금지 — 한글이 든 UTF-8 배치는 cmd 파서를 깨뜨린다(`docs/learning/2026-07-16-korean-batch-utf8-desync.md`). 한국어 문구는 ps1에만 쓰고, ps1은 UTF-8 BOM을 유지한다.
...
15. **수정구역 주석 청소** — `✏️ 수정구역` 주석을 잡음으로 보고 지우거나, 한 파일에서만 이름을 바꾸거나, 새 콘텐츠 블록을 수정구역 없이 추가한다. 이 주석은 사용자의 셀프 편집 시스템이자 `나중에 나 혼자 볼 설명서.md`의 색인이다 (check_lang_sync는 주석을 검사하지 않으므로 어긋나도 PASS가 뜬다 — 도구를 믿지 말 것). → 관례 14.
...
사용자용 가이드 2개: 루트의 `lang_edit.md` — en/jp 텍스트·`<br>`·언어별 여백을 사용자가 직접 편집하는 step-by-step 절차. 루트의 `나중에 나 혼자 볼 설명서.md` — 수정구역에서 텍스트 고치기 → `업데이트.bat`으로 배포하는 전체 절차 + 오류 해결표 + git 되돌리기 (관례 14의 시스템을 사용자 눈높이로 설명한 문서 — 시스템을 바꾸면 이 문서도 같이 갱신).
