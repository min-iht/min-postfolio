...
통한 이유: 모든 요소가 **같은 하나의 길이 값**에서 파생되므로 요소별 반올림 편차가 사라진다. `min()` 캡 덕에 기준 폭 이상에선 `--u = 1px`이라 기존 px 값과 100% 동일(무손실 전환). 배율 축소 시 크기가 고정되는 것들은 명시 필수: background-size, `<img>` width, transform 거리, font-size. (처음엔 보험으로 앞판과 같은 이미지를 li 배경 겹으로도 깔았지만 **롤백** — img의 투명 영역으로 서브픽셀 어긋난 같은 그림이 비쳐 선이 이중으로/번져 보였다. 반투명·투명이 있는 이미지는 같은 자리에 이중으로 그리지 마라.) 100vw-거터 충돌은 해당 페이지가 스크롤바를 절대 안 보이므로 `scrollbar-gutter: auto`로 해제해 100vw == body를 복원.

## 후속 (2026-07-16) — --u만으로는 부족했다

노트북(뷰포트 <1330px 또는 125%/150% 배율 = 분수 DPR)에서 같은 이음새에 흰 줄 재발.
--u 공통 단위로 CSS 값은 정합해도(실측 차이 0.004px), **background는 페인트 단계에서
position·size를 각각 디바이스 픽셀로 스냅하고 이웃 요소는 레이아웃 단계에서 반올림**되어
두 경로가 1디바이스픽셀 갈라질 수 있다. 해법: butt-joint에 1px 블리드 —
`background-size` 세로를 `calc(68 * var(--u) + 1px)`로 늘려 배경이 앞판 아래로
1px 들어가게 한다 (반올림이 어느 방향으로 튀어도 안전, 앞판이 겹침을 덮는다).
canvas 실측으로 upper.webp 마지막 행이 100% 불투명(벽 선 포함)임을 확인해
블리드가 벽 윤곽선을 그대로 연장함을 검증했다.

## 일반화된 교훈

1. **배율 축소에서 맞닿은 그래픽이 어긋나면 `zoom`부터 의심하라.** zoom은 배경/자식/크기 미지정 img를 각각 따로 반올림한다. 해법은 길이형 변수(`--u: min(1px, calc(100vw / 기준폭))`) + `calc(N * var(--u))` 전면 적용. 단 이것도 background 페인트 반올림 vs 레이아웃 반올림 차이는 못 막는다 — **분수 배율에서 살아남아야 하는 butt-joint는 값 정합에 의존하지 말고 1px 겹침(bleed)을 설계에 넣어라.**
